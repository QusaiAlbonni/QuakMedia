        ...
        <div @if ($overflow) style="max-height:250px" @endif>
            <div>
                ...
                @foreach ($post->comments as $comment)
                    <commentedit oct="{{$comment->comment_text}}" img="{{ auth()->user()->profile->image }}" id="commentEdit{{ $comment->id }}"
                        cid="{{ $comment->id }}" comrep="comment" style="display: none;">
                    </commentedit>
                        ...
                        <div class="row bg-white rounded-2 p-2 shadow m-2 ">
                            ...
                            <div class="col-10 px-2">
                                <div class="d-flex justify-content-between w-100">
                                    ...
                                    @auth
                                        @if (Auth::user()->id === $comment->user_id && !$overflow)
                                            <commentdropdown cid="{{ $comment->id }}" comrep="comment"></commentdropdown>
                                        @endif
                                    @endauth
                                </div>
                                <p style="overflow: hidden" id="commenttext{{$comment->id}}">{{ $comment->comment_text }}
                                </p>
                                        ...
                                        <div class="row rounded-2 p-2 shadow m-2 w-75"
                                            ...
                                            <div class="col-10 px-2">
                                                <div class="d-flex justify-content-between">
                                                    ...
                                                    @auth
                                                        @if (Auth::user()->id === $reply->user_id && !$overflow)
